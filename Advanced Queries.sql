...
SELECT 
    ...
    COUNT(p.id_pagamento) AS frequencia_visitas -- Conta o nº total de pagamentos realizados pelo cliente
FROM Clientes c                    -- Tabela principal: lista de todos os clientes
LEFT JOIN Pagamentos p              -- Junta com pagamentos (LEFT JOIN para incluir clientes sem compras)
...
SELECT 
    c.id_cliente,                   -- ID do cliente
    c.nome,                         -- Nome do cliente
    ROUND(AVG(qtd_por_compra), 2) AS volume_medio_consumo -- Média de unidades por compra (arredondado a 2 casas decimais)
FROM (
    -- Subconsulta para calcular a quantidade total de produtos por compra
    SELECT 
        p.id_cliente,               -- ID do cliente
        SUM(ip.quantidade) AS qtd_por_compra -- Soma da quantidade de produtos comprados nessa compra
    FROM Pagamentos p
    JOIN Itens_Pagamento ip 
         ON p.id_pagamento = ip.id_pagamento -- Liga pagamento com os seus itens
    GROUP BY p.id_pagamento, p.id_cliente -- Agrupa por compra para calcular total de unidades por transação
) sub
JOIN Clientes c ON c.id_cliente = sub.id_cliente -- Junta com a tabela de clientes para trazer os nomes
